/**
 * Script para testar a integração do Flow com o Roo Code
 */
...
const FlowConfig = {
    TOKEN_EXPIRY_MINUTES: 55, // 55 minutes (safe margin from 60 minutes)
...
const FLOW_TENANT = "seu-tenant";
const FLOW_CLIENT_ID = "seu-client-id";
const FLOW_CLIENT_SECRET = "seu-client-secret";
const FLOW_APP_TO_ACCESS = "llm-api";
...
async function authenticate() {
    try {
        ...
        const authUrl = `${FLOW_BASE_URL}${FlowConfig.AUTH_PATH}`;
...
async function getProviderModels(token, providerId) {
    try {
        ...
        const url = `${FLOW_BASE_URL}${FlowConfig.API_PATH}/models/${providerId}?capabilities=system-instruction,chat-conversation`;

...
export class FlowHandler extends BaseProvider implements ApiHandler {
  ...
  private validateOptions() {
    const requiredOptions = ['flowBaseUrl', 'flowTenant', 'flowClientId', 'flowClientSecret', 'flowAppToAccess'] as const
  ...
  override async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
    ...
    try {
      ...
      const flowMessages = messages.map((msg) => {
        ...
        } else if (msg.content && typeof msg.content === "object" && "text" in (msg.content as Record<string, unknown>) && typeof (msg.content as { text: unknown }).text === "string") {
      ...
      if (!payload.stream) {
        ...
        const text = endpoint === '/google/generateContent'
        ...
        switch (status) {
          ...
          default:
            if (status && status >= 500) {
              throw new Error(`Flow API server error (${status}): ${errorMessage || 'Internal server error'}`)
            }
        }
...
private async *retryWithDifferentModel(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
